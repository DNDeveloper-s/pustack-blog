...
function SignalsMobileRef(
  {
    startAt,
    _signals: _serverSignals,
  }: {
    startAt?: string | string[] | undefined | null;
    _signals: any;
  },
  ref: any
) {
  ...
  const _serverFormedSignals: Signal[] = useMemo(() => {
  ...
  const hasSignals = signals?.length && signals?.length > 0;
  ...
  return (
    <div
      ...
      ref={ref}
    >
      <div className="w-full max-w-[720px] mx-auto pt-[10px] px-3 pb-[80px] mb-2">
        {hasSignals && (
          <>
...
const SignalsMobile = forwardRef(SignalsMobileRef);

export default SignalsMobile;
