...
import EventItem, { EventItemHeader } from "./EventItem";
import EventItemDesktop, { EventItemDesktopHeader } from "./EventItemDesktop";
...
export default function EventsEntry() {
  ...
  return (
    <div className="overflow-x-auto bg-lightPrimary">
      {isMobileScreen ? <EventItemHeader /> : <EventItemDesktopHeader />}
      {/* <button
            onClick={() => {
              setFilters((c) => ({
                ...c,
        ...
        events?.map((event: Event) =>
          isMobileScreen ? (
            <EventItem
              ...
              isSelected={selectedKeys.includes(event.id as string)}
              key={event.id}
              event={event}
            />
          ) : (
